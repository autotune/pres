resource "aws_instance" "prod_pub_apses2" {
  ...
  vpc_security_group_ids = ["${aws_security_group.prod-apse2.id}"]
  ...
  provisioner "local-exec" {
      command="echo \"wp_db_hostname: ${aws_db_instance.apse2.address}\" > ./wordpress-nginx-2/group_vars/localhost"
  }

  provisioner "file" {
        source = "./wordpress-nginx-2"
  ...
  depends_on = ["aws_db_instance.apse2"]
}
